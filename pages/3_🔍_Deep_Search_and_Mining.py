...
setup_page("Search")
make_footer()
ask_tab, search_tab = st.tabs(["Ask Your Knowledge Base (Deep Search)", "Mining"])
...
async def process_ask_query(question, strategy_model, answer_model, final_answer_model, notebook_id=None):
    ...
    async for chunk in ask_graph.astream(
        input=dict(
            question=question,
            notebook_id=notebook_id,  # Add notebook_id to the input
        ),
        config=dict(
            configurable=dict(
...
with ask_tab:
    ...
    # Add notebook selection
    col1, col2 = st.columns(2)
    with col1:
        search_scope = st.radio("Search scope:", ["All Notebooks", "Selected Notebooks"])
    
    # Initialize notebook_id
    notebook_id = None
    
    if search_scope == "Selected Notebooks":  # Note: Fixed typo here (plural)
        with col2:
            all_notebooks = Notebook.get_all(order_by="name asc")
            selected_notebook = st.multiselect(
                "Select a notebook:",  # Clearer label
                options=all_notebooks,
                format_func=lambda x: x.name,
                help="Choose which notebook to limit your search to"  # Updated help text
            )

            notebook_id = [notebook.id for notebook in selected_notebook] if selected_notebook else None
    ...
    # )
    answer_model = strategy_model
    final_answer_model = strategy_model
    ask_bt = st.button("Ask")
    placeholder = st.container()

    async def stream_results():
        async for chunk in process_ask_query(
            question, strategy_model, answer_model, final_answer_model,notebook_id
        ):
            if "agent" in chunk:
    ...
    # Check if an answer has been generated before displaying the save form
