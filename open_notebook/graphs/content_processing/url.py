...
async def extract_url_bs4(url: str):
    ...
    try:
        ...
        return {
            ...
            "domain": (
                urlparse(url).netloc if not url.startswith("<!DOCTYPE html>") else None
            ),
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"https://r.jina.ai/{url}") as response:
            ...
            if text.startswith("Title:") and "\n" in text:
                ...
                logger.debug(
                    f"Processed url: {url}, found title: {title}, content: {content[:]}..."
                )
                return {"title": title, "content": content}
            else:
                logger.debug(
                    f"Processed url: {url}, does not have Title prefix, returning full content: {text[:]}..."
                )
                return {"content": text}
