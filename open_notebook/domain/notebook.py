...
class Notebook(ObjectModel):
    ...
    def delete(self) -> bool:
        """
        Delete this notebook and perform cascading deletion of related content
        that is not shared with other notebooks.
        ...
        try:
            ...
            for source in self.sources:
                # Delete only the relationship between this notebook and the source
                repo_query(f"DELETE reference WHERE out={self.id} AND in={source.id};")
                
                # Check if source is still related to any other notebooks
                remaining_notebooks = repo_query(f"SELECT * FROM reference WHERE in={source.id};")
                
                # If not related to any notebooks, delete source and related data
                if not remaining_notebooks:
                    # Delete source embeddings
                    repo_query(f"DELETE source_embedding WHERE source={source.id};")
                    
                    # Delete source insights
                    repo_query(f"DELETE source_insight WHERE source={source.id};")
            ...
            for note in self.notes:
                # Delete only the relationship between this notebook and the note
                repo_query(f"DELETE artifact WHERE out={self.id} AND in={note.id};")
                
                # Check if note is still related to any other notebooks
                remaining_notebooks = repo_query(f"SELECT * FROM artifact WHERE in={note.id};")
                
                # If not related to any notebooks, delete the note
                if not remaining_notebooks:
                    note.delete()
            
            # For chat sessions
            for chat_session in self.chat_sessions:
                # Delete only the relationship between this notebook and the chat session
                repo_query(f"DELETE refers_to WHERE out={self.id} AND in={chat_session.id};")
                
                # Check if chat session is still related to any other notebooks
                remaining_notebooks = repo_query(f"SELECT * FROM refers_to WHERE in={chat_session.id};")
                
                # If not related to any notebooks, delete the chat session
                if not remaining_notebooks:
                    chat_session.delete()
            ...
            logger.error(f"Error in cascading delete for notebook {self.id}: {str(e)}")
            logger.exception(e)
            raise DatabaseOperationError(f"Failed to delete notebook and related content")
