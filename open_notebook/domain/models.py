...
class DefaultModels(RecordModel):
    ...
    # Use Field to provide more configuration and allow extra fields
    default_chat_model: Optional[str] = Field(default=None)
    default_transformation_model: Optional[str] = Field(default=None)
    large_context_model: Optional[str] = Field(default=None)
    default_text_to_speech_model: Optional[str] = Field(default=None)
    default_speech_to_text_model: Optional[str] = Field(default=None)
    default_embedding_model: Optional[str] = Field(default=None)
    default_tools_model: Optional[str] = Field(default=None)

    # Allow extra fields
    model_config = ConfigDict(
        extra="allow", populate_by_name=True, from_attributes=True  # Allow extra fields
    )

    def update(self):
        """
        Custom update method to persist changes to the database
        """
        from open_notebook.database.repository import repo_upsert

        # Convert to dictionary, removing ClassVar
        data = {
            k: v
            for k, v in self.model_dump().items()
            if k != "record_id" and v is not None
        }
    ...
    def get_instance(cls):
        """
        Retrieve or create a new instance of DefaultModels
        """
        from open_notebook.database.repository import repo_query

        # Try to fetch existing record
        results = repo_query(f"SELECT * FROM {cls.record_id};")

        if results:
            # If record exists, create instance from existing data
