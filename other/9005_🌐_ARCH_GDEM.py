...
def process_responses(data):
    st.subheader("Gene-Disease Evidence Overview")
    ...
    for index, row in data.iterrows():
        ...
        st.markdown(f"<h4 style='color: green;'>GDEM analyses for {first_column_value}:</h4>", unsafe_allow_html=True)
        
        row_responses = {"Health Condition": first_column_value}
    ...
    response_df = pd.DataFrame(responses_list).set_index("Health Condition")
...
def find_phase3_column(columns):
    for column in columns:
        # Check if 'Phase3' is part of the column name
        if "Phase 3+" in column:
            return column
    return None
...
def run_analysis(data):
    ...
    session_vars = ['health_condition_list', 'health_condition_list_input', 'selected_values', 'min_values', 'sort_by_col', 'num_rows']
    for var in session_vars:
        if var not in st.session_state:
            if var in ['health_condition_list', 'selected_values', 'min_values']:
    ...
    def process_health_conditions():
        clear_filter_states()
        if st.session_state['health_condition_list_input']:
            health_condition_list = re.split(r'[,\n]+', st.session_state['health_condition_list_input'].strip())
            st.session_state['health_condition_list'] = [hc.strip() for hc in health_condition_list if hc.strip()]

    def clear_health_conditions():
        st.session_state['health_condition_list'] = []
        st.session_state['health_condition_list_input'] = ''
        clear_filter_states()
    ...
    # Dynamic detection of the Phase3+ column
    phase3_col = find_phase3_column(filtered_data.columns)

    # Keep only the "Include only rows where Phase3+ is zero" checkbox
    only_zero = st.checkbox("Include only rows where Phase3+ is zero")

    # Ensure that this UI is placed above other input components
    if phase3_col:
        # Apply the filtering based on the checkbox state
        if only_zero:
            filtered_data = filtered_data[filtered_data[phase3_col] == 0]
    else:
        st.warning("The 'Phase3+' column, or similar, is not present in the data. Please check your dataset.")


    st.session_state['health_condition_list_input'] = st.text_area(
        "Paste a list of Health Conditions to filter (comma or newline separated):",
        value=st.session_state['health_condition_list_input'],
        height=100
    )

    col1, col2, col3 = st.columns(3)
    with col1:
        st.button("Submit Health Conditions", on_click=process_health_conditions)
    with col2:
        st.button("Clear Health Conditions", on_click=clear_health_conditions)
    with col3:
        st.button("Clear Numerical Filters", on_click=clear_filter_states)       
    if st.session_state['health_condition_list']:
        filtered_data = filtered_data[filtered_data['Health Condition'].isin(st.session_state['health_condition_list'])]
    ...
    # Add filter for MedDRA columns
    def filter_meddra_column(column, filtered_data):
        search_term = st.text_input(f"Search in {column}:", key=f"search_{column}")
        if search_term:
            # Include try-except block to handle potential errors
            try:
                filtered_data = filtered_data[filtered_data[column].str.contains(search_term, case=False, na=False)]
            except Exception as e:
                st.error(f"Error filtering MedDRA column '{column}': {e}")
    ...
    def handle_filter_column(column, filtered_data):
        # Safely drop NaN values before extracting unique integer values
    ...
    if columns:
        sort_options = [col for col in columns if col != 'Health Condition']
    ...
    st.session_state['num_rows'] = st.slider(
        "Select maximum number of health conditions to include in the analysis:",
    ...
    with col2:
        st.download_button(
            ...
            data=trimmed_data.to_csv(index=False).encode('utf-8'),
    ...
    with col4:
        if st.session_state.get('download_source', False) and 'original_data' in st.session_state:
            st.download_button(
                label="Original Dataset as CSV",
                data=st.session_state['original_data'].to_csv(index=False).encode('utf-8'),
...
def display_dropdown_and_file_upload(options_data, auth_token):
    if options_data is not None:
        if 'name' in options_data.columns and 'url' in options_data.columns:
            ...
            options.insert(0, "Select a gene symbol")
            options.append("Upload an alternate UTF-8 CSV or xlsx GDEM file")

            # Set the index to 0 to start with the placeholder
            selected_option = st.selectbox("**Gene datasets:**", options, index=0)
            
            # Check if the selected option is valid (i.e., not the placeholder)
            if selected_option == "Upload an alternate UTF-8 CSV or xlsx GDEM file":
                uploaded_file = st.file_uploader("Upload file here", type=["csv"])
                if uploaded_file is not None:
                    process_uploaded_csv_file(uploaded_file)

            elif selected_option != "Select a gene symbol" and auth_token:
...
def process_uploaded_csv_file(uploaded_file):
    if uploaded_file is not None:
        try:
            data = pd.read_csv(uploaded_file)
            run_analysis(data)
...
def main():
    ...
    st.markdown("<h1 style='color:green;'>GDEM Assistant</h1>", unsafe_allow_html=True)
    make_footer()
    auth_token = get_token()
    options_csv_path = '/home/cdsw/pages/ref_files/gdem_options.csv'
